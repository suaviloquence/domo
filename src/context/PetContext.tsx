...
export function PetProvider({ children }: { children: React.ReactNode }) {
    ...
    const [ownedAccessories, setOwnedAccessories] = useState<AccessoryId[]>(["none"]);
    const [equippedAccessory, setEquippedAccessory] = useState<AccessoryId>("none");
    ...
    useEffect(() => {
        (async () => {
            ...
            setOwnedAccessories(savedOwned ? (JSON.parse(savedOwned) as AccessoryId[]) : ["none"]);
            setEquippedAccessory(savedEquipped ? (savedEquipped as AccessoryId) : "none");
  ...
  const resetPet = async () => {        // reset pet and moneyy 
